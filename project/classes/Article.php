...
class Article
{
    ...
    protected function validate()
    {
        ...
        if ($this->published_at != '') {
            ...
            //     $this->errors[] = 'Invalid de and time';
    ...
    public static function getCat($conn,$id)
    {
        $sql ="SELECT
        article.*,category.name AS c_name
    FROM
        article
    LEFT JOIN category ON article.category_id = category.id WHERE article.id = :id";
    $stmt=$conn->prepare($sql);
    $stmt->bindValue(':id',$id,PDO::PARAM_INT);
    $stmt->execute();
    return $stmt->fetchAll(PDO::FETCH_ASSOC);
    ...
    FROM
        category
    JOIN article ON article.category_id = category.id WHERE article.id = :id";
    $stmt=$conn->prepare($sql);
    $stmt->bindValue(':id',$this->id,PDO::PARAM_INT);
    $stmt->execute();
    return $stmt->fetchAll(PDO::FETCH_ASSOC);
